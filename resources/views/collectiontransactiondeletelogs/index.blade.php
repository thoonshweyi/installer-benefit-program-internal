        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Collection Transaction Delete Log</h4>
                    </div>
                </div>
            </div>
            {{-- {{ dd(request()->query() ) }} --}}
            {{-- <div class="col-lg-12 mb-2">
                <form action="{{ route('collectiontransactiondeletelogdeletelogs.search') }}" method="GET">
                ...
                <div class="rounded mb-3 table-container">
                    <table class="table table-hover mb-0 tbl-server-info" id="lucky_draw_list">
                        ...
                        <tbody id="tabledata" class="ligth-body">
                            @foreach ($collectiontransactiondeletelogs as $idx=>$collectiontransactiondeletelog)
                                <tr>
                                    <td>{{ ++$idx }}</td>
                                    <td>{{ $collectiontransactiondeletelog->actionuser->name }}</td>
                                    <td>{{ $collectiontransactiondeletelog->actionbranch->branch_name_eng }}</td>
                                    <td>{{ $collectiontransactiondeletelog->document_no  }}</td>
                                    <td>{{ $collectiontransactiondeletelog->installer_card_card_number  }}</td>
                                    <td>{{ $collectiontransactiondeletelog->invoice_number  }}</td>
                                    <td>{{ $collectiontransactiondeletelog->total_points_collected  }}</td>
                                    <td>{{ number_format($collectiontransactiondeletelog->total_save_value,0,'.',',') }} <span class="ms-4">MMK</span></td>
                                    <td>{{ Carbon\Carbon::parse($collectiontransactiondeletelog->collection_date)->format('M d Y') }}</td>
                                    {{-- <td>
                                        @if($collectiontransactiondeletelog->isDeleteAuthUser() && $collectiontransactiondeletelog->allowDelete())
                                            <form action="{{ route('collectiontransactiondeletelogdeletelogs.destroy',$collectiontransactiondeletelog->uuid) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="d-flex justify-content-center">
                        {{ $collectiontransactiondeletelogs->appends(request()->all())->links('pagination::bootstrap-4') }}
                    </div>
                    <div class="myloader">
