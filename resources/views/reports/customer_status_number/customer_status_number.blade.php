        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">{{__('report.customer_status_number')}}</h4>
                    </div>
                </div>
            </div>
        ...
        var table = $('#lucky_draw_list').DataTable({
            ...
            columns: [{
                    ...
                    render: function(data, type, row) {
                        return `<a href="/ticket_history_detail/${row.id}" class="normal_status">${data}</a>`;
                    }
                },
                {
                    data: 'start_date',
                    ...
                    orderable: true,
                    render: function(data, type, row) {
                        return `<a href="/lucky_draws/${row.id}/edit" class="normal_status">${data}</a>`;
                    }
                },
                {
                    data: 'end_date',
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        if(data == 1){
                            return `<a href="/lucky_draws/${row.id}/edit" class="normal_status">Active</a>`;
                        }else if(data == 2){
                            return `<a href="/lucky_draws/${row.id}/edit" class="reject_status">Inactive</a>`;
                        }else if(data == 3){
                            return `<a href="/lucky_draws/${row.id}/edit" class="reject_status">Pending</a>`;
                        }
                    }
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                                <div class="d-flex align-items-center list-action">
                                    <a class="badge bg-secondary mr-2" data-toggle="tooltip" data-placement="top" title="Edit" data-original-title="Edit"
                                        href="/reports/customer_status_number/${row.uuid}"><i class="ri-eye-line mr-0"></i></a>

                                        <a class="badge bg-info mr-2" data-toggle="tooltip" data-placement="top" title="Chart_Report" data-original-title="Edit"
                                        href="/reports/customer_status_number/${row.uuid}/graph"><i class="ri-line-chart-line mr-0"></i></a>
                                        @can('export-report')
                                    <a class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="Excel_Report" data-original-title="Edit"
                                        href="/reports/customer_status_number_export/${row.uuid}"><i class="ri-file-excel-2-line mr-0"></i></a>
                                        @endcan
                                </div>`
                    }
                }
