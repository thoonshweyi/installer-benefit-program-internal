                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Edit Point Promotion</h4>
                        </div>
                    </div>
                    @if (count($errors) > 0)
                    ...
                    @endif
                    <div class="card-body">
                    <form action="{{ route('pointpromos.update',$pointpromotion->uuid) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input name="name" type="text" class="form-control" value="{{old('name',$pointpromotion->name)}}" placeholder="Enter Promotion Name"/>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="pointperamount">Point Per Amount<span class="cancel_status">*</sapn> </label>
                                    <input type="number" name="pointperamount" id="pointperamount"  class="form-control pointperamount" value="{{old('pointperamount',$pointpromotion->pointperamount)}}" placeholder="Enter Amount for 1 point" />
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select id="status" name="status" class="form-control ">
                                        ...
                                        <option value="1" {{ old('status',$pointpromotion->status) == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="2" {{ old('status',$pointpromotion->status) == 2 ? 'selected' : '' }}>Inactive
                                        </option>
                                        @endcan
                                        <option value="3" {{ old('status',$pointpromotion->status) == 3 ? 'selected' : '' }}>Pending
                                        </option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                @php
                                    ...
                                    $allBranchesSelected = count($pointpromotion->branches->pluck('branch_id')->toArray() ?: []) === $branches->count();
                                    // dd($allBranchesSelected);
                                @endphp
                                <div class="form-group">
                                    ...
                                    <input type="checkbox" class="checkbox-input" name="select_all_branch"
                                        id="select_all_branch" {{ ($allBranchesSelected) ? 'checked' : ''}}>
                                    ...
                                    <select name="branch_id[]" id="branch_id" class="form-control " multiple>
                                        ...
                                        <option value="{{ $branch->branch_id }}"
                                            {{ in_array($branch->branch_id, old("branch_id",$pointpromotionbranches->toArray()) ?: []) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input name="start_date" id="start_date" type="date" class="form-control"
                                        ...
                                        value="{{ old('start_date',$pointpromotion->start_date) ?? date('Y-m-d')}}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input name="end_date" id="end_date" type="date" class="form-control"
                                        ...
                                        value="{{ old('end_date',$pointpromotion->end_date) ?? date('Y-m-d')}}">
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div id="pointrules" class="form-group">
                                    ...
                                    {{-- @if(old("category_id") === null) --}}
                                        @foreach ($pointrules as $idx => $pointrule)
                                            <div class="row p-2 border rounded-lg">
                                                <div class="col-md-3 mb-2 mb-md-0">
                                                    {{-- {{ dd($pointrule->pointrulegroups[0]->group_id) }} --}}
                                                    <select name="category_id[]" id="category_id" class="form-control category_id">
                                                        <option disabled selected>Choose Category</option>
                                                        @foreach($categories as $category)
                                                        <option value="{{ $category->maincatid }}" {{ $category->maincatid ==  $pointrule->category_id ? 'selected' : '' }}>
                                                            {{ $category->name }}
                                                        </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-md-3 mb-2 mb-md-0">
                                                    {{-- {{dd($pointrule->pointrulegroups->pluck('group_id'))}} --}}

                                                    <select name="group_id[{{$idx}}][]" id="" class="form-control group_id" multiple>
                                                        {{-- <option disabled selected>Choose Group</option> --}}
                                                        @foreach($pointrule->groupsfilterBycategory_id() as $group)
                                                        <option value="{{ $group->product_group_id }}"  {{ in_array($group->product_group_id, $pointrule->pointrulegroups->pluck('group_id')->toArray() ?: []) ? 'selected' : '' }}>
                                                    ...
                                                    @php
                                                        // Check if all branches are selected
                                                        $allGroupsSelected = count($pointrule->pointrulegroups->pluck('group_id')->toArray() ?: []) === $pointrule->groupsfilterBycategory_id()->count();
                                                        // dd($allGroupsSelected);
                                                    @endphp
                                                    <input type="checkbox" class="checkbox-input select_all_groups" name=""
                                                    id="select_all_groups{{$idx}}" {{($allGroupsSelected) ? 'checked' : ''}}>
                                                    <label for="select_all_groups{{$idx}}">Select All Group</label>
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="number" name="redemption_value[]" id="redemption_value" class="redemption_value form-control" placeholder="Enter Redemption Value"  value="{{ $pointrule->redemption_value }}"/>
                                                </div>
                                                <div class="col">
                                                    <div class="d-flex">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="radio d-inline-block mr-2">
                                        <input type="radio" name="discon_status" id="radio1" value='1'
                                            @if($pointpromotion->discon_status == 1) checked @endif>
                                        <label for="radio1">Include</label>
                                    </div>
                                    <div class="radio d-inline-block mr-2">
                                        <input type="radio" name="discon_status" id="radio2" value='2'
                                            @if($pointpromotion->discon_status == 2) checked @endif >
                            ...
                            <div class="col-md-12 mb-2">
                                ...
                                <textarea name="remark" id="remark" class="form-control" rows="4" placeholder="Write Something....">{{ old('remark',$pointpromotion->remark) }}</textarea>
                            </div>
                            <div class="col-md-12">
...
$(document).ready(function() {
    ...
    $(document).on("change", ".group_id", function() {
        ...
        selectedOptionCount = $(this).closest('.col-md-3').find('.group_id option:selected').length;
        allgroupoptions = $(this).closest('.col-md-3').find('.group_id option').length;

        {{-- console.log(allgroupoptions); --}}


        if(selectedOptionCount < allgroupoptions){
            $(this).closest('.col-md-3').find('.select_all_groups').prop('checked',false);
        }
    });
    ...
    $(document).on("click", ".ruleaddbtn", function() {
        ...
        $("#pointrules").append(`
            <div class="row p-2 border rounded-lg">
                <div class="col-md-3 mb-2 mb-md-0">
                    <select name="category_id[${ruleidx}]" id="category_id" class="form-control category_id">
                ...
                <div class="col-md-3">
                    <input type="number" name="redemption_value[${ruleidx}]" id="redemption_value" class="redemption_value form-control" placeholder="Enter Redemption Value"/>
                </div>
                <div class="col">
                    <div class="d-flex">
                        <button type="button" id="ruleremovebtn" class="ruleremovebtn btn btn-danger text-center d-flex justify-content-center align-items-center mr-1"><i class="fas fa-minus"></i></button>
